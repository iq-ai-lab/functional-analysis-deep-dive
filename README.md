...
| [04. 스펙트럼 정리 — 컴팩트·자기수반](./ch4-compact-spectral/04-spectral-theorem-compact.md) | $T = \sum\lambda_n\langle\cdot,e_n\rangle e_n$ (가산 실수 고유값, 0 외에는 유한 다중도), 증명: Rayleigh 지수 최대화 → 첫 번째 고유쌍, 귀납으로 나머지, Mercer 정리와의 직접 연결, NumPy로 Gaussian 커널 고유함수 분해 |
...
scipy==1.11.0
matplotlib==3.8.0
sympy==1.12             # 기호 미분·적분·방정식 풀이 (Ch6 Euler-Lagrange 유도 등)
...
pip install numpy==1.26.0 scipy==1.11.0 matplotlib==3.8.0 \
            sympy==1.12 scikit-learn==1.3.0 cvxpy==1.4.0 jupyter==1.0.0
